...
def test_1_connection_and_access() -> TestResult:
    ...
    try:
        ...
        if count > 0:
            return TestResult("Connection & Access", True, f"Connected. DRILLING_TIME has {count:,} rows.")
        else:
...
def test_2_file_scanner() -> TestResult:
    ...
    try:
        ...
        if files:
            return TestResult("FileScanner", True, f"Found {len(files)} files: {[f.name for f in files]}")
        else:
...
def test_3_schema_profiler() -> TestResult:
    ...
    try:
        ...
        if profile and profile.columns:
            return TestResult("SchemaProfiler", True, f"Profiled {len(profile.columns)} columns, {profile.row_count:,} rows")
        else:
...
def test_4_use_case_parser() -> TestResult:
    ...
    try:
        ...
        if result:
            return TestResult("UseCaseParser", True, f"Task: {result.primary_task.value}, ML: {result.ml_enabled}, Search: {result.search_enabled}")
        else:
...
def test_5_data_scanner() -> TestResult:
    ...
    try:
        ...
        if total > 0:
            return TestResult("DataScanner", True, f"Found {len(stage_assets)} stage files, {1 if table_asset else 0} tables")
        else:
...
def test_6_completeness_validator() -> TestResult:
    ...
    try:
        ...
        result = validator.check_required_columns(
            TABLE_FQN,
            ["WELL_NAME", "TIMESTAMP", "RATE_OF_PENETRATION_M_H"]
        )
        conn.close()
        
        passed = result.get("passed", False)
        return TestResult("CompletenessValidator", passed, f"Cols: {result.get('found', 0)}/{result.get('required', 0)}")
    except Exception as e:
