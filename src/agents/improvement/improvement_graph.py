...
def _fallback_classification(state: ImprovementState) -> Dict[str, Any]:
    ...
    if any(kw in request_lower for kw in ["page", "button", "ui", "display", "show", "hide", "layout", "chart"]):
...
def _fallback_impact(state: ImprovementState) -> Dict[str, Any]:
    ...
    agent_map = {
        ...
        ImprovementType.DATA_CHANGE.value: ["parquet_processor", "document_chunker", "validation_agent"],
        ImprovementType.ML_CHANGE.value: ["feature_store_builder", "ml_model_builder"],
        ImprovementType.INTEGRATION_CHANGE.value: ["cortex_search_builder", "semantic_model_generator", "app_code_generator"],
...
def generate_plan(state: ImprovementState) -> Dict[str, Any]:
    ...
    if request_type == ImprovementType.DATA_CHANGE.value:
        phases.append({
            ...
            "agents": [a for a in agents_to_run if a in ["parquet_processor", "document_chunker"]],
            "parallel": True,
        })
        phases.append({
            "phase": "validation",
    ...
    if request_type in [ImprovementType.UI_CHANGE.value, ImprovementType.BUG_FIX.value] or "app_code_generator" in agents_to_run:
        phases.append({
...
def notify_user(state: ImprovementState) -> Dict[str, Any]:
    ...
    notification_parts = [
        f"Your improvement request has been processed!",
        "",
...
def run_improvement_pipeline(
    request: str,
    plan_id: str,
    current_app_state: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
