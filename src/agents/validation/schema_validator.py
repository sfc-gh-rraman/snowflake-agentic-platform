...
class SchemaValidator:
    ...
    def check_naming_convention(
        ...
        if len(parts) >= 3:
            _db, _schema, table = parts[0], parts[1], parts[-1]
        else:
            table = parts[-1]
        ...
        pattern = self.NAMING_PATTERNS.get(
            self.naming_convention, self.NAMING_PATTERNS["SCREAMING_SNAKE"]
        )
        violations = []
            ...
            for r in results:
                ...
                for keyword, expected_types in self.EXPECTED_TYPES.items():
                    if keyword in col_name:
                        type_matches = any(
                            exp_type in data_type.upper() for exp_type in expected_types
                        )
                        if not type_matches:
    ...
    def validate(
        ...
        if not type_check.get("passed"):
            issues.append(
                f"Type inconsistencies found: {len(type_check.get('inconsistencies', []))}"
            )
