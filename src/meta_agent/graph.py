...
def run_from_config(
    ...
    """
    from src.config import load_use_case_yaml, UseCaseConfig
    from src.generators import generate_ddls, generate_app
...
def _extract_requirements(config) -> Dict[str, Any]:
    ...
    return {
        ...
        "ml_models": [
            {"name": m.name, "task": m.task.value, "target": m.target}
...
def _build_execution_plan(config) -> Dict[str, Any]:
    ...
    phases.append({
        ...
        "steps": [
            ...
            {"action": "create_stages", "count": len(config.data.structured) + len(config.data.unstructured)},
    ...
    if config.data.unstructured:
        phases.append({
            ...
            "steps": [
                {"action": "chunk_documents", "assets": [a.name for a in config.data.unstructured]},
                {"action": "create_embeddings"},
                {"action": "create_search_services", "services": [
                    s.name for s in config.cortex_services.search
                ]},
            ],
        })
